...
RUN curl https://getcomposer.org/download/1.0.0-alpha11/composer.phar \
    -o /usr/local/bin/composer && chmod +x /usr/local/bin/composer

RUN curl https://phar.phpunit.de/phpunit-5.2.9.phar \
    -o /usr/local/bin/phpunit && chmod +x /usr/local/bin/phpunit

RUN curl http://get.sensiolabs.org/sami.phar
    -o /usr/local/bin/sami && chmod +x /usr/local/bin/sami

RUN curl http://get.sensiolabs.org/php-cs-fixer.phar
